...
class LeggedRobotE2E(LeggedRobot):
    ...
    def _setup_ball_trajectory(self, env_ids):
        ...
        # Ball comes from dist away. Curriculum: gradually increase max distance
        min_dist = getattr(self.cfg.commands, 'ball_dist_min', 1.5)
        max_dist_limit = getattr(self.cfg.commands, 'ball_dist_max', 8.0)
        # Increase over 100k steps
        curric_factor = min(1.0, self.common_step_counter / 100000.0)
        max_dist = min_dist + (max_dist_limit - min_dist) * curric_factor
    ...
    def _draw_debug_vis(self):
        ...
        # Draw physical ball position and velocity in viewer
        sphere_geom = gymutil.WireframeSphereGeometry(0.1, 8, 8, None, color=(1, 0, 0)) # Red ball
        for i in range(self.num_envs):
            x = self.ball_pos[i, 0].item()
            y = self.ball_pos[i, 1].item()
            z = self.ball_pos[i, 2].item()
            sphere_pose = gymapi.Transform(gymapi.Vec3(x, y, z), r=None)
            gymutil.draw_lines(sphere_geom, self.gym, self.viewer, self.envs[i], sphere_pose)
            
            # Velocity vector (scaled for visibility)
            vx = self.ball_vel[i, 0].item()
            vy = self.ball_vel[i, 1].item()
            vz = self.ball_vel[i, 2].item()
            p1 = gymapi.Vec3(x, y, z)
            p2 = gymapi.Vec3(x + vx * 0.2, y + vy * 0.2, z + vz * 0.2)
