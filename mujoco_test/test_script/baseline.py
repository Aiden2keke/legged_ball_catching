...
actor.load_state_dict(torch.load(actor_dir + "/actor_oraclebl-9000.pth")) # baseline policy
actor = actor.to(device)
actor.eval()
proprio_encoder = MLPEncoder(input_dim=45*15)
proprio_encoder.load_state_dict(torch.load(proprio_encoder_dir + "/proprio_oraclebl-9000.pth")) # baseline policy
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and time.time() - start < 5000:
        ...
        noise_strength = 0.02  # The intensity of the noise. The larger the value, the stronger the noise.
        noise = np.random.randn(*proprio_observation.shape) * noise_strength  # Generates noise of the same shape as proprio_observation
        proprio_observation += noise  # Add noise
